...
def parse_modis_dates(url, dates, product, out_dir, ruff=False):
    ...
    html = str(urllib2.urlopen(req).readlines())
    lines = html.split('</a>')
    ...
    for line in lines:
        ...
        if line.find("href") >= 0 and line.find("[DIR]") >= 0:
            ...
            if ruff:
                try:
                    modis_date = time.strftime("%Y%j", time.strptime(the_date, "%Y.%m.%d"))
                except ValueError:
                    continue
            ...
            for line in html:
                if line.find(tile) >= 0 and line.find(".hdf") >= 0 and line.find(".hdf.xml") < 0:
                    ...
                    if download:
                        ...
                        with open(os.path.join(out_dir, fname), 'wb') as local_file_fp:
                            shutil.copyfileobj(urllib2.urlopen(req), local_file_fp)
                            if verbose:
                                LOG.info("Done!")
                    else:
                        if verbose:
